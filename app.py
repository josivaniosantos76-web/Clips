...
def processar_cortes():
    try:
        ...
        # Recebe os dois arquivos do celular
        arquivo_video = request.files.get('video')
        imagem_fundo = request.files.get('fundo')
        
        if not arquivo_video or not imagem_fundo:
            return jsonify({"erro": "Faltam arquivos essenciais (Vídeo ou Imagem de Fundo)"}), 400
            
        # Salva a imagem de fundo
        bg_path = os.path.join(UPLOAD_FOLDER, 'fundo.png')
        imagem_fundo.save(bg_path)
        
        # Salva o vídeo enviado da galeria
        video_original = os.path.join(UPLOAD_FOLDER, 'downloaded.mp4')
        arquivo_video.save(video_original)
        
        print("[SERVER] Vídeo recebido com sucesso da galeria. Iniciando limpeza...")
        
        # Limpeza de workspace de execuções anteriores
